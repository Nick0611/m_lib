...
function xi = IntrplBezier_xi(a,Interval,varargin)
n = nargin-2;
% 方案一
% if n == 2
...
if n == 2
    ...
    z0 = varargin{1}+a*(varargin{2}-varargin{1});
    zend = varargin{2}-a*(varargin{2}-varargin{1});
%     z0 = varargin{1};
%     zend = varargin{2};
    num = max(4,round(norm(varargin{2}-varargin{1}) / Interval));
    xi{1} = IntrplLee_xi(num,varargin{1},z0,zend,varargin{2}); % 李群轨迹插值函数
end
if n == 3
%     z0 = varargin{1}*expm(a*logm(pinv(eye(4))*varargin{2}));
    z0 = varargin{1}+a*(varargin{2}-varargin{1});
    z{1} = varargin{1}-a*(varargin{3}-varargin{1});
    z{2} = varargin{1}+a*(varargin{3}-varargin{1});
%     zend = varargin{3}*expm(-a*logm(pinv(varargin{2})*eye(4)));
    zend = varargin{3}-a*(varargin{3}-varargin{2});
    num = max(4,round(norm(varargin{2}-varargin{1}) / Interval));
    xi{1} = IntrplLee_xi(num,varargin{1},z0,z{1},varargin{2}); % 李群轨迹插值函数
    num = max(4,round(norm(varargin{3}-varargin{2}) / Interval));
    xi{2} = IntrplLee_xi(num,varargin{2},z{2},zend,varargin{3}); % 李群轨迹插值函数
end
if n > 3
    z = coder.nullcopy(cell(1,n));
    z0 = varargin{1};
    z{1} = varargin{2}-a*(varargin{3}-varargin{1});
    num = max(4,round(norm(varargin{2}-varargin{1}) / Interval));
    xi{1} = IntrplLee_xi(num,varargin{1},z0,z{1},varargin{2}); % 李群轨迹插值函数
    for i = 1:n-3
        num= max(4,round(norm(varargin{i+2}-varargin{i+1}) / Interval));
        z{i+1} = varargin{i+1}+a*(varargin{i+2}-varargin{i});
        z{i+2} = varargin{i+2}-a*(varargin{i+3}-varargin{i+1});
        xi{i+1} = IntrplLee_xi(num,varargin{i+1},z{i+1},z{i+2},varargin{i+2}); % 李群轨迹插值函数
    end
    num = max(4,round(norm(varargin{i+3}-varargin{i+2}) / Interval));
    z{i+3} = varargin{i+2}+a*(varargin{i+3}-varargin{i+1});
    zend = varargin{i+3};
    xi{i+2} = IntrplLee_xi(num,varargin{i+2},z{i+3},zend,varargin{i+3}); % 李群轨迹插值函数
end
% Revision 20230712
for i = 1:size(xi,2)-1
    xi{i}(end)=[];
