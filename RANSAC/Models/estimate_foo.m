function [Theta, k] = estimate_foo(X)

% [Theta k] = estimate_foo(X)
%
% DESC:
...
%
% OUTPUT:
% Theta             = estimated parameter vector Theta = H(:). If the 
%                     estimation is not successful return an empty
%                     vector. i.e. Theta = [];
...
N = size(X, 2);
if (N < k)
    error('At least k point correspondences are required');
end;
