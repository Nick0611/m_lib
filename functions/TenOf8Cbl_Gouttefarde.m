...
vf = vli;
I = find(round(tmin-tp,6) <= round(N*vli,6) & round(N*vli,6) <= round(tmax-tp,6)); % round的原因：将浮点数四舍五入到特定的精度，以消除小的数值误差。
% scatter3S(vf);
% textS(vf,'text','0');
...
%     disp(itr_num);
    ...
    alpha_k(alpha_k<1e-6) = nan; % 三线交于一点的舍去
